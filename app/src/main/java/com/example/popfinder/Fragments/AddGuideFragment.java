...
public class AddGuideFragment extends Fragment {
    ...
    private EditText editName, editPhone, editLocation;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        btnSave.setOnClickListener(v -> {
            if (editName.getText().length() > 0 && editPhone.getText().length() > 0 && editLocation.getText().length() > 0) {
                saveData(editName.getText().toString(), editPhone.getText().toString(), editLocation.getText().toString());
            } else {
